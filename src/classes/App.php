...
class App {
	...
	public function __construct(array $config = []){
		...
		$this->logger->pushHandler(new ErrorLogHandler(ErrorLogHandler::OPERATING_SYSTEM, self::LOG_LEVEL));
	 ...
	 */
	public static function makeGallery($context, string $dir, array $options = []){
		$dir = rtrim($dir, '/');
		$options += [
			'width' => 200,
		...
		foreach (scandir($file_dir) as $file){
			...
			elseif (static::fileIsYaml($file_dir.'/'.$file)) {
				...
				$yaml_data = array_merge_recursive($yaml_data, Yaml::parseFile($file_dir.'/'.$file));
			}
		}
	...
	private function loadCurrentBuild(){
		...
		$build = [];
		foreach (array_diff(scandir($this->config['cache_dir'].'/build/data'), ['..', '.']) as $index){
	 ...
	 */
	private function findAllFiles($base_dir, $options = [], &$files = []){
		$options = array_merge([
			'include_all' => false,
			'save_to_queue_id' => false,
		], $options);
		$dirname = rtrim($base_dir, '/').'/';
		$content = array_diff(scandir($dirname), ['..', '.']);
		foreach ($content as $file){
			if ($options['include_all'] || !in_array(basename($file), $this->config['exclude_files']) && !$this->checkFileVsExcludeRegex($file)){
				$file = $dirname.$file;
				$add_this_file = true;
				if (is_dir($file)){
		...
		foreach ($queue_buffer as $item){
			...
			switch ($op){
				...
				case static::BUILD_OP_BUILD:
					...
					$this->logger->info(sprintf("%d\t%s", ++$this->build['config']['built'], $this->processContentToBuild($file)));
				break;
				default:
	...
	public function processContentToBuild($file){
		...
		if (static::fileIsMarkdown($file)){
			...
			if (isset($this->build['pages'][$page_index]) && ($source_page = $this->build['pages'][$page_index])){
				...
				if (isset($source_page['meta']['pagination'])){
					...
					if ($source_page['meta']['pagination']['data']=='pages'){
						...
						$pages = array_filter($this->build['pages'], function ($page) use ($source_page){
							return ($page['id']!=$source_page['id'] // not the calling page
								&& !empty($page['meta']['title']) // has title
								&& empty($page['meta']['hidden']) // not hidden
								&& (empty($source_page['meta']['index_options']['subpage_prefix']) || preg_match(sprintf('|^%s|', preg_quote($source_page['meta']['index_options']['subpage_prefix'], '|')), $page['path'])) // correct page path prefix
								&& (empty($source_page['meta']['index_options']['templates']) || in_array($page['meta']['template'], $source_page['meta']['index_options']['templates'])) // correct template
							);
						});
				...
				foreach ($pages_to_build as $page){
					...
					$page_params['current_page']['content'] = $this->twig->render('_convert_twig_in_content.twig', $page_params + [
					...
					$rendered_page = $this->twig->render($page['meta']['template'], $page_params);
					if (!empty($build_file)){
		...
		return empty($action) ? '' : sprintf("%s\t%s\t%f", $action, $this->stripContentDir($file), microtime(true) - $start);
	}
	...
	private function outputBuild(){
		...
		$this->logger->info(sprintf('Moving build dir %s to output dir %s', $this->build['config']['output_dir'], $this->config['output_dir']));
	...
	public function watch(){
		...
		$this->logger->info(sprintf('Content change detected at %s - old checksum %s, new checksum %s', date('c'), $checksum, $new_checksum));
