...
class Zettelkasten implements ContentFilePlugin
{
    ...
    public static function processHtml(string $html, Build $build): string
    {
        ...
        $new_html = preg_replace_callback('/href\s*=\s*\"(\d+)\"/', static function ($matches) use ($zettel_index) {
            $zid = (int)$matches[1];
            /** @var non-falsy-string $href */
            $href = $matches[0];
            if (!isset($zettel_index[$zid])) {
                return $href;
            }
            $target_path = $zettel_index[$zid];
            $href = str_replace((string)$zid, $target_path, $href);
            Utils::getLogger()
        ...
        if (is_null($new_html)) {
            throw new RuntimeException('Failed to process HTML');
        }
